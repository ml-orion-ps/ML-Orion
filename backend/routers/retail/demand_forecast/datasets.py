...
from services.ml_service import run_retail_demand_fc_service
...
@router.post("/{dataset_id}/demand_forecast")
def demand_forecast(dataset_id: int, body: dict = {}, db: Session = Depends(get_db)):
    ...
    models_to_run = body.get("modelsToRun") or body.get("models_to_run") or ["Exponential Smoothing", "SARIMA", "SARIMAX", "XGBoost","LightGBM"]
    custom_run_name = (body.get("name") or "").strip()
    ...
    result = run_retail_demand_fc_service(
        training_rows,
        models_to_run=models_to_run,
        custom_feature_names=custom_feature_names,
    )
    if not result.get("success"):
        raise HTTPException(status_code=500, detail=result.get("error", "Demand Forecasting failed"))

    feature_report = dict(ds.feature_report) if isinstance(ds.feature_report, dict) else {}
    summary = result.get("summary", {})
    unique_combinations = result.get("unique_combinations")   # top-level, not inside summary
    feature_report["DemandForecasting"] = {
        "summary": summary,
        "unique_combinations": unique_combinations,
        "preview": (result.get("data") or [])[:50],           # SKU-level rows, not global predictions
    }
    ...
    fi_raw = summary.get("Feature_Importance") or {}
    feature_importance_list = _sanitize_for_json([
        {"name": k, "importance": v}
        for k, v in fi_raw.items()
        if isinstance(v, (int, float)) and v > 0
    ])
    sku_wmapes = result.get("sku_wmapes", [])
    storage.create_ml_model(db, {
        "name": custom_run_name or f"DemandForecast - {ds.name} - {best_model}",
        ...
        "r2": metrics.get("R2"),
        "actualUnits": totals.get("actualUnits"),
        "forecastUnits": totals.get("forecastUnits"),
    })
...
def run_eda(dataset_id: int, usecase: str = Query("retail_demand_forecast"), db: Session = Depends(get_db)):
    ds = storage.get_dataset(db, dataset_id)
    if not ds:
        raise HTTPException(status_code=404, detail="Dataset not found")
...
def validate_custom_feature_endpoint(dataset_id: int, body: dict, db: Session = Depends(get_db)):
    ...
    result = validate_custom_feature(feature, col_names, use_case="retail_demand_forecast")
    return result
...
def preview_custom_feature(dataset_id: int, body: dict, db: Session = Depends(get_db)):
    ...
    preview = build_preview_rows(rows[:100], [feature], use_case="retail_demand_forecast")
...
def feature_suggestions(dataset_id: int, db: Session = Depends(get_db)):
    ...
    suggestion_specs = [
        (
            "avg_cateogory_price_store_week",
            "AVG(Final Retail Price) by P3 Category, store, Week_Date",
            "Captures average Category-level price movement within a store-week.",
            "high",
            0.060,
        ),
        (
            "own_cat_price_index",
            "Final Retail Price / (AVG(Final Retail Price) by P3 Category, store, Week_Date)",
            "Measures SKU price positioning versus the category average.",
            "high",
            0.054,
        ),
        (
            "avg_sales_for_week_or_month",
            "AVG(Net Shipped) by Week_Date",
        ...
        (
            "seasonality_index",
            "(AVG(Net Shipped) by Week_Date) / AVG(Net Shipped)",
            "Captures recurring seasonal demand patterns.",
            "high",
            0.052,
        ),
