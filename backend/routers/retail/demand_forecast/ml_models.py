...
def latest_features(dataset_id: int | None = Query(None), model_id: int | None = Query(None), db: Session = Depends(get_db)):
    ...
    if not models:
        return {"features": [], "algorithm": None, "modelId": None, "modelName": None}
    model = models[0]
    fi = model.feature_importance or []
    return {"features": fi, "algorithm": model.algorithm, "modelId": model.id, "modelName": model.name}
