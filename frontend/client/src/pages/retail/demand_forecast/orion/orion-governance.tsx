...
export default function OrionGovernancePage() {
  ...
  const { data: gov, isLoading } = useQuery<any>({ queryKey: ["/api/retail/demand_forecast/orion/governance"] });
  ...
  const compliantCount = registry.filter(r => {
    const cc = r.complianceChecks || {};
    return cc.dataLineage && cc.metricsRecorded && cc.featureDocumented && r.approvalStatus === "approved";
  }).length;

  return (
    <OrionLayout title="Governance" subtitle="Model registry, compliance, and full audit trail" isLoading={isLoading}>
      <div className="space-y-4">
        <OrionNav current="/retail/demand_forecast/orion/governance" basePath="/retail/demand_forecast/orion" />
          ...
          <div className="bg-card border rounded-lg overflow-hidden">
            ...
            {registry.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground text-xs">
                ...
                <div className="mt-2"><a href="/retail/demand_forecast/orion/experiments" className="text-primary underline">Go to Experiments →</a></div>
              </div>
            ) : (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b bg-muted/30">
                      {["Model", "Algorithm", "Dataset", "WMAPE", "MAPE", "RMSE", "Status", "Trained", "Deployed", "Approval", "Approver", "Rows Scored"].map(h => (
                  ...
                  <tbody>
                    {registry.map(m => (
                      <tr key={m.id} className="border-b hover:bg-muted/10" data-testid={`row-registry-${m.id}`}>
                        ...
                        <td className="px-3 py-2 font-mono font-bold text-primary">{m.wmape != null ? m.wmape.toFixed(1) + "%" : "—"}</td>
                        <td className="px-3 py-2 font-mono">{m.mape != null ? m.mape.toFixed(1) + "%" : "—"}</td>
                        <td className="px-3 py-2 font-mono">{m.rmse != null ? m.rmse.toFixed(1) : "—"}</td>
                        ...
                        <td className="px-3 py-2 font-mono">{(m.rowsScored ?? 0).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-card border rounded-lg overflow-hidden">
              ...
              {filteredAudit.length === 0 ? (
                <div className="p-8 text-center text-muted-foreground text-xs">
                  No audit events recorded yet. Events are logged automatically when you train, deploy, forecast, or delete models.
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
        ...
        {tab === "compliance" && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <KpiCard label="Features Documented" value={registry.filter(r => r.hasFeatureImportance).length} color="green" />
            </div>
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b bg-muted/30">
                        ...
                        <th className="text-center px-3 py-2 text-muted-foreground font-medium whitespace-nowrap">Data Lineage</th>
                        <th className="text-center px-3 py-2 text-muted-foreground font-medium whitespace-nowrap">Metrics</th>
                        <th className="text-center px-3 py-2 text-muted-foreground font-medium whitespace-nowrap">Features</th>
                        <th className="text-center px-3 py-2 text-muted-foreground font-medium whitespace-nowrap">Approval</th>
                        <th className="text-center px-3 py-2 text-muted-foreground font-medium whitespace-nowrap">Overall</th>
                      </tr>
                    </thead>
                    <tbody>
                      {registry.map(m => {
                        ...
                        const allPass = cc.dataLineage && cc.metricsRecorded && cc.featureDocumented && hasApproval;
                        return (
                          <tr key={m.id} className="border-b hover:bg-muted/10" data-testid={`row-compliance-${m.id}`}>
                            ...
                            <td className="px-3 py-2 whitespace-nowrap"><div className="flex justify-center"><CompliancePip ok={!!cc.dataLineage} /></div></td>
                            <td className="px-3 py-2 whitespace-nowrap"><div className="flex justify-center"><CompliancePip ok={!!cc.metricsRecorded} /></div></td>
                            <td className="px-3 py-2 whitespace-nowrap"><div className="flex justify-center"><CompliancePip ok={!!cc.featureDocumented} /></div></td>
                            <td className="px-3 py-2 whitespace-nowrap"><div className="flex justify-center"><CompliancePip ok={hasApproval} /></div></td>
                            <td className="px-3 py-2 whitespace-nowrap">
            ...
            <div className="bg-card border rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                {[
                  ...
                  { label: "Metrics Recorded", desc: "WMAPE, MAPE, and RMSE available" },
                  { label: "Features Documented", desc: "Tree-based models (XGBoost, LightGBM) only; N/A models pass automatically" },
