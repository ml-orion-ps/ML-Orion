...
function findColumn(columns: any[], candidates: string[]) {
  const normalizedCandidates = candidates.map(normalizeColumnName);
  return columns.find((column: any) =>
    normalizedCandidates.includes(normalizeColumnName(String(column.name || "")))
  );
}

function estimateTotalFromMean(stat: any, totalRows: number) {
  const mean = Number(stat?.mean);
  return Number.isFinite(mean) && totalRows > 0 ? Math.round(mean * totalRows * 100) / 100 : null;
}
...
export default function BaselineOrionDataPage() {
  ...
  const selectedEntityColumn = useMemo(
    () => pickFirstColumn(builderColumns, ["store", "Style Code", "P3 Category"]),
    [builderColumns],
  );
  const selectedTimeColumn = useMemo(
    () => pickFirstColumn(builderColumns, ["Week_Date"]),
    [builderColumns],
  );
  const selectedNumericColumn = useMemo(
    () => pickFirstColumn(builderColumns, ["Final Retail Price", "Final Cost", "price", "Net Shipped", "Net Invoiced (USD"], isNumericColumn),
    [builderColumns],
  );
  ...
  const edaMut = useMutation({
    mutationFn: (id: number) => apiRequest("POST", `${API_BASE}/datasets/${id}/eda?usecase=retail_demand_forecast`),
  ...
  // const dsEda = selectedDataset ? ((selectedDataset as any).edaReport?.["retail_demand_forecast"] ?? null) : null;
  ...
  const dsEda = rawEda ? (rawEda["retail_demand_forecast"] || rawEda) : null;

const dsQuality = selectedDataset ? ((selectedDataset as any).qualityReport || (selectedDataset as any).quality_report) : null;
  const rawActiveEda = edaSource === "live" ? eda : dsEda;
  const activeEda = useMemo(() => {
    if (!rawActiveEda || edaSource === "live") return rawActiveEda;
    const columns = ((selectedDataset as any)?.columns || []) as any[];
    const selectedRowCount = Number((selectedDataset as any)?.rowCount ?? (selectedDataset as any)?.row_count ?? 0);
    const overview = rawActiveEda.overview || {};
    const totalRows = Number(overview.totalRows ?? 0) > 0 ? overview.totalRows : selectedRowCount;
    if (!columns.length) {
      return {
        ...rawActiveEda,
        overview: {
    ...
    const hasNumericStats = Object.keys(rawActiveEda.numericStats || {}).length > 0;
    const hasCatStats = Object.keys(rawActiveEda.catStats || {}).length > 0;

    const numericStats = hasNumericStats ? rawActiveEda.numericStats : columns
      .filter((col: any) => col.type === "numeric" || col.type === "number")
      .reduce((acc: Record<string, any>, col: any) => {
        const nullPercent = Number.parseFloat(col.nullPercent ?? "0") || 0;
        acc[col.name] = {
          ...(col.numericStats || {}),
          nullCount: col.nullCount ?? 0,
          uniqueCount: col.uniqueCount,
          completeness: Math.max(0, Math.round((100 - nullPercent) * 10) / 10),
        };
        return acc;
      }, {});

    const catStats = hasCatStats ? rawActiveEda.catStats : columns
      .filter((col: any) => col.type !== "numeric" && col.type !== "number")
      .reduce((acc: Record<string, any>, col: any) => {
        acc[col.name] = {
          nullCount: col.nullCount ?? 0,
          uniqueCount: col.uniqueCount,
          top: (col.categoricalStats?.topValues || []).map((item: any) => ({
    ...
    const skuColumn = findColumn(columns, ["Style Code", "P4 Style", "SKU", "SKU ID", "Product ID", "Item ID"]);
    const storeColumn = findColumn(columns, ["store", "Store", "Store ID", "Location", "Location ID"]);
    const weekColumn = findColumn(columns, ["Week_Date", "Week Date", "week", "date", "Date"]);
    const brandColumn = findColumn(columns, ["brand", "Brand"]);
    const categoryColumn = findColumn(columns, ["P3 Category", "Category", "Product Category"]);
    const channelColumn = findColumn(columns, ["sales_channel_short_name", "Sales Channel", "Channel"]);
    const regionColumn = findColumn(columns, ["region", "Region", "Market"]);
    const unitsColumn = findColumn(columns, ["Net Shipped", "Sales Units", "Units", "Quantity"]);
    const netSalesColumn = findColumn(columns, ["Net Invoiced (USD)", "Net Sales", "Revenue", "Sales"]);
    const priceColumn = findColumn(columns, ["Final Retail Price", "Retail Price", "Price"]);
    const discountColumn = findColumn(columns, ["Discount", "Discount Value", "Promo Discount"]);
    ...
    return {
      ...
      overview: {
        ...overview,
        totalRows,
        skuCount: overview.skuCount ?? skuColumn?.uniqueCount,
        storeCount: overview.storeCount ?? storeColumn?.uniqueCount,
        weekCount: overview.weekCount ?? weekColumn?.uniqueCount,
        brandCount: overview.brandCount ?? brandColumn?.uniqueCount,
        categoryCount: overview.categoryCount ?? categoryColumn?.uniqueCount,
        channelCount: overview.channelCount ?? channelColumn?.uniqueCount,
        regionCount: overview.regionCount ?? regionColumn?.uniqueCount,
        totalSalesUnits: overview.totalSalesUnits ?? estimateTotalFromMean(numericStats[unitsColumn?.name], totalRows),
        totalNetSales: overview.totalNetSales ?? estimateTotalFromMean(numericStats[netSalesColumn?.name], totalRows),
        avgPrice: overview.avgPrice ?? numericStats[priceColumn?.name]?.mean,
        avgDiscount: overview.avgDiscount ?? numericStats[discountColumn?.name]?.mean,
        features: overview.features ?? columns.length,
        numericFeatures: overview.numericFeatures ?? Object.keys(numericStats).length,
        categoricalFeatures: overview.categoricalFeatures ?? Object.keys(catStats).length,
      },
    };
  }, [edaSource, rawActiveEda, selectedDataset]);
    ...
    return features
      .map((feature: any) => {
        ...
        return {
          ...
          description: feature.description || "Signal used by the selected forecasting model.",
        };
      })
  ...
  return (
    <OrionLayout title="Data Hub" subtitle="Dataset registry, EDA, and Feature Engineering" isLoading={edaLoading}>
      <div className="space-y-4">
        <OrionNav current="/retail/demand_forecast/orion/data" basePath="/retail/demand_forecast/orion" />
              ...
              <>
                  ...
                  <div className="space-y-4">
                    ...
                    {isForecast ? (
                      <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          ...
                          <KpiCard label="Total Revenue" value={activeEda.overview.totalNetSales != null ? `₹${Number(activeEda.overview.totalNetSales).toLocaleString()}` : "—"} color="green" />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="bg-card border rounded-lg p-4">
                        {isForecast ? (
                          <>
                            <h4 className="text-s font-semibold mb-3 uppercase tracking-wider text-muted-foreground">Promo Columns Detected</h4>
                        
                            <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                              <div className="bg-muted/30 rounded p-2 text-center">
                                <div className="text-xs text-muted-foreground">Numeric Features</div>
                                <div className="font-bold text-primary">{activeEda.overview.numericFeatures}</div>
                              </div>
                              <div className="bg-muted/30 rounded p-2 text-center">
                                <div className="text-xs text-muted-foreground">Categorical Features</div>
                ...
                {edaTab === "Bivariate" && (
                  isForecast ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {/* Top SKUs by Volume */}
                      {(() => {
                        const stat = (activeEda.catStats || {})["Style Code"];
                        if (!stat?.top?.length) return null;
                        const maxCount = Math.max(...stat.top.map((x: any) => x.count || 0), 1);
                        return (
                          <div className="bg-card border rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                              <h4 className="text-xs font-semibold text-muted-foreground uppercase">Top SKUs by Volume</h4>
                              <span className="text-[9px] text-muted-foreground shrink-0 ml-2">{stat.uniqueCount} unique</span>
                            </div>
                            <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                              {stat.top.slice(0, 15).map((item: any, idx: number) => (
                                <div key={idx} className="flex items-center gap-2 text-xs">
                                  <span className="w-28 truncate text-muted-foreground font-mono text-[10px]">{item.label}</span>
                                  <div className="flex-1 h-2.5 bg-muted rounded overflow-hidden">
                                    <div className="h-full bg-blue-500 rounded" style={{ width: `${(item.count / maxCount) * 100}%` }} />
                                  </div>
                                  <span className="w-14 text-right font-mono text-[10px]">{item.count?.toLocaleString()}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      {(() => {
                        const stat = (activeEda.catStats || {})["P3 Category"];
                        if (!stat?.top?.length) return null;
                        const maxCount = Math.max(...stat.top.map((x: any) => x.count || 0), 1);
                        return (
                          <div className="bg-card border rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                              <h4 className="text-xs font-semibold text-muted-foreground uppercase">Sales Units by Category</h4>
                              <span className="text-[9px] text-muted-foreground shrink-0 ml-2">{stat.uniqueCount} unique</span>
                            </div>
                            <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                              {stat.top.slice(0, 15).map((item: any, idx: number) => (
                                <div key={idx} className="flex items-center gap-2 text-xs">
                                  <span className="w-28 truncate text-muted-foreground text-[10px]">{item.label}</span>
                                  <div className="flex-1 h-2.5 bg-muted rounded overflow-hidden">
                                    <div className="h-full bg-amber-500 rounded" style={{ width: `${(item.count / maxCount) * 100}%` }} />
                                  </div>
                                  <span className="w-14 text-right font-mono text-[10px]">{item.count?.toLocaleString()}</span>
                                </div>
                              ))}
                      ...
                      {/* Store Distribution */}
                      {(() => {
                        const stat = (activeEda.catStats || {})["store"];
                        if (!stat?.top?.length) return null;
                        const maxCount = Math.max(...stat.top.map((x: any) => x.count || 0), 1);
                        return (
                          <div className="bg-card border rounded-lg p-4">
                            <div className="flex items-center justify-between mb-3">
                              <h4 className="text-xs font-semibold text-muted-foreground uppercase">Store Distribution</h4>
                              <span className="text-[9px] text-muted-foreground shrink-0 ml-2">{stat.uniqueCount} unique</span>
                            </div>
                            <div className="space-y-1.5 max-h-64 overflow-y-auto pr-1">
                              {stat.top.slice(0, 15).map((item: any, idx: number) => (
                                <div key={idx} className="flex items-center gap-2 text-xs">
                                  <span className="w-28 truncate text-muted-foreground font-mono text-[10px]">{item.label}</span>
                                  <div className="flex-1 h-2.5 bg-muted rounded overflow-hidden">
                                    <div className="h-full bg-purple-500 rounded" style={{ width: `${(item.count / maxCount) * 100}%` }} />
                                  </div>
                                  <span className="w-14 text-right font-mono text-[10px]">{item.count?.toLocaleString()}</span>
                                </div>
                              ))}
                  ...
                  isForecast ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-card border rounded-lg overflow-hidden">
                          <div className="px-4 py-3 border-b">
                            ...
                            <p className="text-[10px] text-muted-foreground mt-0.5">Total records per store (top 20)</p>
                          </div>
                          <div className="overflow-y-auto max-h-56">
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="border-b bg-muted/30">
                                  {["Store", "Count", "Share"].map(h => <th key={h} className="text-left px-3 py-2 text-muted-foreground">{h}</th>)}
                                </tr>
                              </thead>
                              <tbody>
                                {(() => {
                                  const items = (activeEda.catStats?.["store"]?.top || []).slice(0, 20);
                                  const maxU = Math.max(...items.map((s: any) => s.count || 0), 1);
                                  const total = items.reduce((sum: number, s: any) => sum + (s.count || 0), 0);
                                  return items.length === 0
                                    ? <tr><td colSpan={3} className="px-3 py-4 text-muted-foreground text-center text-[10px]">No store column detected.</td></tr>
                                    : items.map((s: any, i: number) => (
                                      <tr key={i} className="border-b hover:bg-muted/10">
                                        <td className="px-3 py-1.5 font-mono text-[10px]">{s.label}</td>
                                        <td className="px-3 py-1.5 font-mono text-[10px]">{s.count?.toLocaleString()}</td>
                                        <td className="px-3 py-1.5 w-32">
                                          <div className="flex items-center gap-1">
                                            <div className="flex-1 h-1.5 bg-muted rounded overflow-hidden">
                                              <div className="h-full bg-primary rounded" style={{ width: `${(s.count / maxU) * 100}%` }} />
                                            </div>
                                            <span className="text-[9px] text-muted-foreground w-8 text-right">{total > 0 ? ((s.count / total) * 100).toFixed(1) : 0}%</span>
                                          </div>
                                        </td>
                                      </tr>
                                    ));
                        ...
                        <div className="bg-card border rounded-lg overflow-hidden">
                          <div className="px-4 py-3 border-b">
                            <h4 className="text-xs font-semibold">SKU Distribution</h4>
                            <p className="text-[10px] text-muted-foreground mt-0.5">Total records per SKU (Style Code, top 20)</p>
                          </div>
                          <div className="overflow-y-auto max-h-56">
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="border-b bg-muted/30">
                                  {["SKU", "Count", "Share"].map(h => <th key={h} className="text-left px-3 py-2 text-muted-foreground">{h}</th>)}
                                </tr>
                              </thead>
                              <tbody>
                                {(() => {
                                  const items = (activeEda.catStats?.["Style Code"]?.top || []).slice(0, 20);
                                  const maxU = Math.max(...items.map((s: any) => s.count || 0), 1);
                                  const total = items.reduce((sum: number, s: any) => sum + (s.count || 0), 0);
                                  return items.length === 0
                                    ? <tr><td colSpan={3} className="px-3 py-4 text-muted-foreground text-center text-[10px]">No SKU column detected.</td></tr>
                                    : items.map((s: any, i: number) => (
                                      <tr key={i} className="border-b hover:bg-muted/10">
                                        <td className="px-3 py-1.5 font-mono text-[10px]">{s.label}</td>
                                        <td className="px-3 py-1.5 font-mono text-[10px]">{s.count?.toLocaleString()}</td>
                                        <td className="px-3 py-1.5 w-32">
                                          <div className="flex items-center gap-1">
                                            <div className="flex-1 h-1.5 bg-muted rounded overflow-hidden">
                                              <div className="h-full bg-amber-500 rounded" style={{ width: `${(s.count / maxU) * 100}%` }} />
                                            </div>
                                            <span className="text-[9px] text-muted-foreground w-8 text-right">{total > 0 ? ((s.count / total) * 100).toFixed(1) : 0}%</span>
                                          </div>
                                        </td>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="bg-card border rounded-lg overflow-hidden">
                          <div className="px-4 py-3 border-b">
                            <h4 className="text-xs font-semibold">Category Distribution</h4>
                            <p className="text-[10px] text-muted-foreground mt-0.5">Total records per P3 Category</p>
                          </div>
                          <div className="overflow-y-auto max-h-56">
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="border-b bg-muted/30">
                                  {["Category", "Count", "Share"].map(h => <th key={h} className="text-left px-3 py-2 text-muted-foreground">{h}</th>)}
                                </tr>
                              </thead>
                              <tbody>
                                {(() => {
                                  const items = activeEda.catStats?.["P3 Category"]?.top || [];
                                  const maxU = Math.max(...items.map((s: any) => s.count || 0), 1);
                                  const total = items.reduce((sum: number, s: any) => sum + (s.count || 0), 0);
                                  return items.length === 0
                                    ? <tr><td colSpan={3} className="px-3 py-4 text-muted-foreground text-center text-[10px]">No category column detected.</td></tr>
                                    : items.map((s: any, i: number) => (
                                      <tr key={i} className="border-b hover:bg-muted/10">
                                        <td className="px-3 py-1.5 text-[10px]">{s.label}</td>
                                        <td className="px-3 py-1.5 font-mono text-[10px]">{s.count?.toLocaleString()}</td>
                                        <td className="px-3 py-1.5 w-32">
                                          <div className="flex items-center gap-1">
                                            <div className="flex-1 h-1.5 bg-muted rounded overflow-hidden">
                                              <div className="h-full bg-emerald-500 rounded" style={{ width: `${(s.count / maxU) * 100}%` }} />
                                            </div>
                                            <span className="text-[9px] text-muted-foreground w-8 text-right">{total > 0 ? ((s.count / total) * 100).toFixed(1) : 0}%</span>
                                          </div>
                                        </td>
                        ...
                        <div className="bg-card border rounded-lg p-4">
                          <h4 className="text-xs font-semibold mb-3 text-muted-foreground uppercase">Numeric Column Stats</h4>
                          {Object.keys(activeEda.numericStats || {}).length === 0 ? (
                            <p className="text-xs text-muted-foreground">No numeric columns detected.</p>
                          ) : (
                            <div className="space-y-3">
                              {Object.entries(activeEda.numericStats || {}).map(([col, stat]: [string, any]) => {
                                const mean = stat.mean ?? 0;
                                const maxVal = Math.max(...Object.values(activeEda.numericStats || {}).map((s: any) => s.mean ?? 0), 1);
                                return (
                                  <div key={col} className="space-y-0.5">
                                    <div className="flex justify-between text-xs">
                                      <span className="text-muted-foreground truncate max-w-[140px]">{col}</span>
                                      <span className="font-mono font-medium">{mean.toLocaleString(undefined, { maximumFractionDigits: 2 })} avg</span>
                                    </div>
                                    <div className="h-2 bg-muted rounded overflow-hidden">
                                      <div className="h-full bg-blue-500 rounded" style={{ width: `${(mean / maxVal) * 100}%` }} />
                                    </div>
                                    <div className="flex gap-3 text-[9px] text-muted-foreground">
                                      <span>min {stat.min?.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
                                      <span>max {stat.max?.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
                                      <span>std {stat.std?.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
                                    </div>
                                  </div>
                                );
                              })}
                      ...
                      {Object.keys(activeEda.numericStats || {}).length > 0 && (
                        <div className="bg-card border rounded-lg p-4">
                          <h4 className="text-xs font-semibold mb-3 text-muted-foreground uppercase">Key Numeric Metrics</h4>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {Object.entries(activeEda.numericStats || {}).slice(0, 4).map(([col, stat]: [string, any]) => (
                              <KpiCard key={col} label={col} value={stat.mean != null ? stat.mean.toLocaleString(undefined, { maximumFractionDigits: 1 }) : "—"} />
                            ))}
                          </div>
                        </div>
                      )}
                ...
                {edaTab === "Time Trends" && (
                  isForecast ? (
                    <div className="bg-card border rounded-lg p-4">
                      {(() => {
                        const trends = activeEda.timeTrends || [];
                        if (trends.length > 0) {
                          const maxUnits = Math.max(...trends.map((t: any) => t.totalUnits || 0), 1);
                          return (
                            <>
                        ...
                        // Fallback: find any date/week-like column in catStats
                        const dateKey = Object.keys(activeEda.catStats || {}).find(k =>
                          /week|date|month|year|period|time/i.test(k)
                        );
                        const dateStat = dateKey ? (activeEda.catStats || {})[dateKey] : null;
                        if (dateStat?.top?.length > 0) {
                          const items = dateStat.top.slice(0, 30);
                          const maxCount = Math.max(...items.map((x: any) => x.count || 0), 1);
                          return (
                            <>
                              <h4 className="text-xs font-semibold mb-4 text-muted-foreground uppercase">Records by {dateKey}</h4>
                              <div className="space-y-3">
                                {items.map((t: any, i: number) => (
                                  <div key={t.label ?? i} className="space-y-0.5">
                                    <div className="flex justify-between text-xs">
                                      <span className="font-mono text-muted-foreground">{t.label}</span>
                                      <span className="text-primary font-mono text-[10px]">{t.count?.toLocaleString()} rows</span>
                                    </div>
                                    <div className="h-4 bg-muted rounded overflow-hidden">
                                      <div className="h-full bg-primary/60 rounded" style={{ width: `${(t.count / maxCount) * 100}%` }} />
                                    </div>
                                  </div>
                                ))}
                              </div>
                        ...
                        return <p className="text-xs text-muted-foreground">No week/date column detected — time trends not available.</p>;
                      })()}
                    </div>
                  ) : (
                    <div className="bg-card border rounded-lg p-4">
                  ...
                  <div className="space-y-4">
                    <div className="bg-card border rounded-lg overflow-hidden">
                      <div className="px-4 py-3 border-b">
                        ...
                        <p className="text-[10px] text-muted-foreground mt-0.5">Blue = positive · Red = negative · Computed across all numeric columns</p>
                      </div>
                      <div className="p-4 space-y-2">
                        {(activeEda.correlationMatrix || []).length === 0 ? (
                          <p className="text-xs text-muted-foreground">
                            {Object.keys(activeEda.numericStats || {}).length < 2
                              ? "At least 2 numeric columns are required to compute correlations. Only 1 or fewer numeric columns were detected."
                              : "No correlation pairs computed — re-run EDA to generate this analysis."}
                          </p>
                    ...
                    {Object.keys(activeEda.numericStats || {}).length > 0 && (
                      <div className="bg-card border rounded-lg p-4">
                        <h4 className="text-xs font-semibold mb-3 text-muted-foreground uppercase">Numeric Columns in Analysis</h4>
                        <div className="flex flex-wrap gap-2">
                          {Object.keys(activeEda.numericStats || {}).map(col => (
                            <span key={col} className="text-[10px] font-mono bg-muted px-2 py-1 rounded">{col}</span>
                          ))}
                        </div>
                      </div>
                    )}
                ...
                {!featuresLoading && selectedFeatureDataset && MODEL_FEATURES.length === 0 && (
                  <div className="bg-card border border-dashed rounded-lg px-4 py-12 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {datasetsModels.length > 0 ? "Run a fresh forecasting experiment to populate model feature scores." : "Train a model on this dataset in the Experiments Lab first."}
                    </p>
                  </div>
                )}
