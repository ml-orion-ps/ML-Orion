...
export default function OrionGovernancePage() {
  ...
  const { data: gov, isLoading } = useQuery<any>({ queryKey: ["/api/retail/kpiAnomaly/orion/governance"] });
  ...
  return (
    <OrionLayout title="Governance" subtitle="Model registry, compliance, and full audit trail" isLoading={isLoading}>
      <div className="space-y-4">
        <OrionNav current="/retail/kpi-anomaly/orion/governance" basePath="/retail/kpi-anomaly/orion" />
          ...
          <div className="bg-card border rounded-lg overflow-hidden">
            ...
            {registry.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground text-xs">
                ...
                <div className="mt-2"><a href="/retail/kpi-anomaly/orion/experiments" className="text-primary underline">Go to Experiments →</a></div>
              </div>
            ) : (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b bg-muted/30">
                      {["Model", "Algorithm", "Dataset", "Anomalies", "KPIs", "Status", "Trained", "Deployed", "Approval", "Approver"].map(h => (
                  ...
                  <tbody>
                    {registry.map(m => (
                      <tr key={m.id} className="border-b hover:bg-muted/10" data-testid={`row-registry-${m.id}`}>
                        ...
                        <td className="px-3 py-2 font-mono font-bold text-primary">{m.totalAnomalies ?? "—"}</td>
                        <td className="px-3 py-2 font-mono">{m.kpisProcessed ?? "—"}</td>
