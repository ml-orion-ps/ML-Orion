...
def run_baseline_prediction(
    ...
    custom_feature_names = [
        ...
        if isinstance(name, str) and name in model_df.columns and name != "sales_units"
    ]
    for col in custom_feature_names:
        series = pd.to_numeric(model_df[col], errors="coerce")
        fill_value = series.median()
        if pd.isna(fill_value):
            fill_value = 0.0
        model_df[col] = series.fillna(fill_value)
    ...
    custom_terms = "\n    + " + "\n    + ".join(f'Q("{col}")' for col in custom_feature_names) if custom_feature_names else ""
...
"""
        ...
        if best_model_name in ["RF", "XGB"]:
            ...
            feature_importance = sorted(
                [
                    ...
                    for col, coef in zip(x.columns, coeffs)
                ],
                key=lambda z: z["importance"],
                reverse=True
            )
    ...
    summary = {
        ...
        "customFeaturesUsed": custom_feature_names,
